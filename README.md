...
#### 📖 Resumo
Este repositório possui uma pipeline automatizada chamada `Terraform Deploy` que permite **provisionar a infraestrutura base do projeto** (vpcs, cluster, etc) sempre que houver um push na branch `main`.
...
```bash
git clone https://github.com/ns-fiap-tc/fiap_hackathon_infra_base
```

2. **Acesse o diretório do repositório clonado, por exemplo**:

```bash
cd fiap_hackathon_infra_base
```
